...
export default function InfonavBwpPayroll() {
  const [startDate, setStartDate] = useState("");
  const [endDate, setEndDate] = useState("");
  const [results, setResults] = useState<InfonavBwpRow[]>([]);
  ...
  const fetchData = async () => {
    ...
    setLoading(true);
    const res = await axios.get("/admin/payroll-infonav-bwp", {
          ...
          <tbody>
            {results.map((r, i) => (
              <tr key={i} className="hover:bg-purple-50">
                ...
                <td className="px-4 py-2 font-semibold">{r.salary.toFixed(2)}</td>
